...
def cmd_run() -> None:
    ...
    ctx = RuntimeContext(settings=settings, broker=broker, journal=Journal(settings.log_dir))
    try:
        ctx.llm  # fail fast on a missing/invalid API key instead of waiting all day
    except Exception as exc:  # noqa: BLE001
        broker.disconnect()
        sys.exit(f"could not initialize LLM '{settings.llm_model}': {exc}")
    try:
        TradingSession(ctx).run_live()
    finally:
        broker.disconnect()
...
def cmd_check_ib() -> None:
    ...
    try:
        ...
        bars = broker.get_bars(settings.symbol, lookback_minutes=30)
        print(f"\nlast {min(20, len(bars))} of {len(bars)} 1-min {settings.symbol} bars:")
        for b in bars[-20:]:
            print(f"  {b.ts:%Y-%m-%d %H:%M}  O{b.open:.2f} H{b.high:.2f} L{b.low:.2f} C{b.close:.2f}")

        prev_high, prev_low = broker.get_prev_day_range(settings.symbol)
        print(f"\nprev day range: high {prev_high} / low {prev_low}")

        chain = broker.get_option_chain(settings.symbol)
        print(f"chain: {len(chain.strikes)} strikes, nearest expirations {chain.expirations[:4]}")

        if bars:
            contract = select_contract(chain, settings.symbol, bars[-1].close, "call", broker.now())
            if contract:
    ...
    if args.csv:
        ...
        try:
            ...
            raw = ib.ib.reqHistoricalData(
                ib._underlying(settings.symbol),
                endDateTime=day,
                durationStr="1 D",
            ...
            prev_high, prev_low = ib.get_prev_day_range(settings.symbol)
